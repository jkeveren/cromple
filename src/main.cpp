...
namespace program_n {
	...
	class error_t {
		...
		// If a user of a unit need low level error access then the user should implement it or the unit should provide a way of injecting low level resources.
		...
		enum reasons {
			...
			reason_other = 1, // Other should be used when an error occured but you don't want to make the machine readable reason part of the external API. For implementation details that are likely to change.
			// none__... and other being 0 and 1 makes them also work as sensible exit codes so you can just exit with the error reason which defaults to other which is 1.
			custom_reason_start, // Start custom reason enums at this int.
		};
		...
		// default to other. If an error occurred, there was a reason but we can.t assume what it is here.
		int reason = reason_other;
		
		error_t(std::string message, int reason = reason_other) : message_stack{message},  reason{reason} {}

		error_t append(std::string message, int reason = reason_other) {
			this->reason = reason;
			message_stack += message + "\n";
			return *this;
		}

		// Prints the message_stack and returns reason int (nice for exiting program).
